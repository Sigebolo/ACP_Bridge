#!/usr/bin/env python3
"""
实施Claude Agent ACP通讯 - 向Claude介绍ACP Bridge项目
"""
...
async def connect_to_claude_acp():
    """连接Claude Agent ACP并发送项目介绍"""
    
    print("[Claude ACP] 启动Claude Agent ACP...")
    ...
    try:
        ...
        if response_line:
            ...
            print(f"[Claude ACP] ✅ 初始化响应: {response}")
        ...
        if response_line:
            response = json.loads(response_line.strip())
            print(f"[Claude ACP] ✅ 会话创建响应: {response}")
...
你好Claude！很高兴通过Agent Client Protocol与你连接！
...
"""
                ...
                while time.time() - start_time < 15:  # 等待15秒（留5秒缓冲）
                    response_line = claude_process.stdout.readline()
        ...
        if claude_process.stderr:
            ...
            print(f"[Claude ACP] Claude错误输出: {error_output}")
...
if __name__ == "__main__":
    asyncio.run(connect_to_claude_acp())
