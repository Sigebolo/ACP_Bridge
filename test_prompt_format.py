#!/usr/bin/env python3
"""
Test sending prompt format instead of messages format to Claude Agent
Based on the error message suggesting it expects 'prompt' field
"""
...
async def test_prompt_format():
    """Test using prompt format (based on error message hint)"""

    print("[Test] Starting Claude Agent ACP...")
    print("[Test] Testing 'prompt' format based on error message hint")
    ...
    try:
        ...
        if response_line:
            ...
            if "result" in response:
                ...
                # 3. 发送prompt格式的消息 (based on error hint)
                test_message = "Hello Claude! Testing prompt format."

                # Based on error: it wants 'prompt' field that is an array
                prompt_request = {
                    "jsonrpc": "2.0",
                    "id": "3",
                ...
                while time.time() - start_time < 10:
                    ...
                    try:
                        ...
                        if "method" in response and response["method"] == "session/update":
                            ...
                            # If this also fails, we know it's not just the format
                            break
                ...
                if claude_response:
                    ...
                    print("[Test] [WARNING] No response from Claude within 10 seconds")
...
if __name__ == "__main__":
    success = asyncio.run(test_prompt_format())
    if success:
        ...
        print("\n[Test] [RESULT] TEST FAILED - Neither format works")
        print("[Test] [CONCLUSION] There may be a deeper issue")